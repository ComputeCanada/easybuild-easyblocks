...
class EB_UCX_Plugins(ConfigureMake):
    ...
    def configure_step(self):
        ...
        if LooseVersion(self.version) >= LooseVersion('1.20.0') and (cudaroot or rocmroot):
            self.makefile_dirs.insert(0, 'tools/perf/lib')
    ...
    def make_module_extra(self, *args, **kwargs):
        ...
        if LooseVersion(self.version) >= LooseVersion('1.22.0'):
            # for >= 1.22.0 we only need UCX_MODULE_DIR
            return txt + self.module_generator.prepend_paths('UCX_MODULE_DIR', 'ucx')
