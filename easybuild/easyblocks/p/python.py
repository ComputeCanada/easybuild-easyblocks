...
class EB_Python(ConfigureMake):
    ...
    def configure_step(self):
        ...
        if tcl and tk:
            ...
            tcl_libdir = os.path.join(tcl, get_software_libdir('Tcl', fs=['libtcl%s.%s'%(tcltk_maj_min_ver, shlib_ext)]))
            tk_libdir = os.path.join(tk, get_software_libdir('Tk', fs=['libtk%s.%s'%(tcltk_maj_min_ver, shlib_ext)]))
